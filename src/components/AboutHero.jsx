...
export default function AboutHero() {
  ...
  // NOTE: The entry animation already left these elements at y: 175.
  // The scroll animation needs to start from this point and move them up.
  
  const scrollTl = gsap.timeline({
    scrollTrigger: {
      trigger: sectionEl,
      start: "top top",
      end: "+=150%", // longer scroll duration
      scrub: 1.5, // Your chosen scrub value
      pin: false,
      
      // You may need to refresh ScrollTrigger after the initial animation
      // ScrollTrigger.refresh() could be added to the end of the first timeline's onComplete.
    },
  });

  // 1. Mountains/Land moving up (Parallax effect)
  // They start at y: 175 (from entry animation) and move to y: -100
  // Total movement = 175 - (-100) = 275 pixels of displacement across 150vh of scroll.
  scrollTl.to(
    [landEl, mountainEl, topRockEl],
    {
      y: -100, // Move from starting y: 175 to a final y: -100 (Total Upward movement)
      ease: "none",
    },
    0 // Start at the beginning of the scroll timeline
  );

  // 2. Make the next section move up with the mountains
  if (nextSection) {
    scrollTl.to(
      nextSection,
      {
        y: -100, // Make the next section move up for a clean transition
        ease: "none",
      },
      0 // Start at the beginning of the scroll timeline
    );
  ...
  // 3. Zoom out the background stars (Optional, enhances parallax)
  scrollTl.to(
    bgRef.current,
    {
      scale: 1.05, // Zoom out slightly from scale 1 (entry end state)
      ease: "none",
    },
    0 // Start at the beginning of the scroll timeline
  );
  ...
  return (
    ...
    >
      ...
      {/*  Navbar */}
        <div
          className="
    absolute bottom-0 left-0 w-full h-[15%]
    pointer-events-none
    bg-gradient-to-t
    from-black
    to-transparent
      ...
      >
        <div>
          ...
          <p className="mt-4 max-w-xl text-sm opacity-90">
            At Sinatra Holding, we envision a world where opportunity <br />{" "}
            meets innovation. Guided by our core belief in sustainable <br />{" "}
            growth, we strive to build companies that create value— <br />{" "}
